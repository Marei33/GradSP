...
for method in methods:
    ...
    for k in range(len(k_range)):
        ...
        for j in range(0,10):
            ...
            training_frame.to_csv(f"Results/Smoothing_param_method={method}_epochs={num_epochs_lambda_param}_{k}_j{j}.csv")
            ...
            # save final results
            filename = '1d-spline_' + x1_name + '_method=' + method + '_basisdim=' + str(basis_dimension) + '_epochs=' + str(num_epochs_lambda_param*num_epochs_splines) + '.npz'
            np.savez(file = os.path.join(dirname, 'Results', filename), reg_param = lambda_param_est, weights = weight_est)

    
    # save optimal lambda value for every data set per method
    lambda_param_est_opt = pd.DataFrame(data=lambda_param_est_opt)
    lambda_param_est_opt.to_csv(f"Results/Smoothing_param_OPT_method={method}_epochs={num_epochs_lambda_param}.csv")
