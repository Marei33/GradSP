...
def penalized_least_squares(y, weights, design_matrix, reg_param, penalty_matrix):      
    ...
    penalty = reg_param * (tf.tensordot(tf.transpose(weights), tf.tensordot(penalty_matrix, weights, axes = 1), axes = 1))  
...
def gcv_2d(y, design_matrix_Z_1, reg_matrix_K_1, reg_param_1, design_matrix_Z_2, reg_matrix_K_2, reg_param_2):
    ...
    design_matrix_Z = np.float32(tf.concat([design_matrix_Z_1, design_matrix_Z_2], axis=1))
    
    G1 = tf.linalg.matmul(a=design_matrix_Z_1, b=design_matrix_Z_1, transpose_a=True) + reg_param_1 * reg_matrix_K_1    
    ...
    G4 = tf.linalg.matmul(a=design_matrix_Z_2, b=design_matrix_Z_2, transpose_a=True) + reg_param_2 * reg_matrix_K_2
...
def gcv_1d(y, design_matrix_Z, reg_matrix_K, reg_param):
    ...
    G = tf.linalg.matrix_transpose(design_matrix_Z) @ design_matrix_Z + reg_param * reg_matrix_K
    G_inv = tf.linalg.inv(G)
    S = design_matrix_Z @ G_inv @ tf.linalg.matrix_transpose(design_matrix_Z)
...
def reml_1d(y, design_matrix_Z, reg_matrix_K, reg_param):
    ...
    reml1 = (alpha + tf.linalg.matmul(a=beta_est, b= (reg_param * reg_matrix_K), transpose_a=True) @ beta_est) / 2*phi

    reml2 = (tf.math.log(tf.linalg.det(G/tf.norm(G, ord='euclidean'))) + G.shape[0]*tf.math.log(tf.norm(G, ord='euclidean')) - G.shape[0]*tf.math.log(phi)  ) /2 
    ...
    reml3 = - tf.cast(np.linalg.matrix_rank(reg_matrix_K), dtype=tf.float32 ) * tf.math.log(reg_param/phi) / 2 # + log(detplus(K)) - for derivative not relevant
