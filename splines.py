...
def single_b_spline(x, knots, degree, index):
    ...
    if degree < 0.5:
        for i in range(len(x)):
            res[i] = 1 * (
                x[i] >= knots[index] and x[i] < knots[index + 1]
            )  # TRUE = 1, FALSE = 0
    ## Recursive call to single.b.spline from inside the same function:
        ...
        res = (
            ...
            * single_b_spline(x, knots, degree - 1, index)
        )
        res = res + (knots[index + degree + 1] - x) / (
            knots[index + degree + 1] - knots[index + 1]
        ) * single_b_spline(x, knots, degree - 1, index + 1)
    return res
...
def build_base(x, degree, knot_type="equi", basis_dimension=None, given_knots=None):
    ...
    if knot_type == "equi" and basis_dimension == None:
        raise ValueError(
            'For knot_type = "equi" the basis dimension has to be specified.'
        )
    ...
    if knot_type == "equi":
        ...
        inner_knots = np.linspace(
            start=min(x), stop=max(x), num=n_inner_knots
        )  # min(x) & max(x) sind knots
        min_dist = min(np.diff(inner_knots))
        left_outer_knots = np.linspace(
            start=min(x) - degree * min_dist, stop=min(x) - min_dist, num=degree
        )
        right_outer_knots = np.linspace(
            start=max(x) + min_dist, stop=max(x) + degree * min_dist, num=degree
        )
...
class pspline:
    ...
    def __init__(
        ...
        if knot_type != "equi" and knot_type != "given":
            raise ValueError(
                'knot_type has to be specified as either "equi" or "given".'
            )
        if knot_type == "equi" and basis_dimension == None:
            raise ValueError(
                'For knot_type = "equi" the basis dimension has to be specified.'
        ...
        if knot_type == "given" and any(given_knots == None):
            raise ValueError(
                'knot_type = "given" was chosen, but no knots were supplied.'
            )
        ...
        if knot_type == "equi":
            ...
            self.design_matrix, self.knots = build_base(
                x, degree=self.degree_bsplines, basis_dimension=self.basis_dimension
            )
        if knot_type == "given":
            self.basis_dimension = len(given_knots) - self.degree_bsplines - 1
            self.design_matrix, self.knots = build_base(
                x,
                degree=self.degree_bsplines,
                knot_type="given",
                given_knots=given_knots,
            )

        self.penalty_matrix = build_penalty_matrix(
            self.penalty_diff_order, dim=self.basis_dimension
        )
